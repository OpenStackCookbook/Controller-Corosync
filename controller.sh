...
cert_subject=/C=US/ST=Unset/L=Unset/O=Unset/CN=${ETH3_IP}
...
# Copy to machine specific certs so we can test individually
sudo cp /etc/keystone/ssl/certs/ca.pem /vagrant/ca-$(hostname -s).pem
sudo cp /etc/keystone/ssl/certs/cakey.pem /vagrant/cakey-$(hostname -s).pem
...
create_endpoints(){
  ...
  export OS_KEY=/vagrant/cakey-$(hostname -s).pem
  export OS_CACERT=/vagrant/ca-$(hostname -s).pem
...
[database]
backend = sqlalchemy
connection = mysql://glance:openstack@${ETH3_IP}/glance
...
cat > /vagrant/openrc-$(hostname -s) <<EOF
export OS_TENANT_NAME=cookbook
export OS_USERNAME=admin
export OS_PASSWORD=openstack
export OS_AUTH_URL=https://${ETH3_IP}:5000/v2.0/
...
##################################### 
# Chapter 12 - Production OpenStack #
# Corosync + Pacemaker              #
#####################################
